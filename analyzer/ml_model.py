...
class MLProcessor:
    ...
    def extract_conclusion(self, text: str) -> str:
        """Extract conclusion section - improved detection with intelligent fallback."""
        ...
        # Generate intelligent conclusion if extraction fails
        return self._infer_conclusion(text)
    
    def _infer_conclusion(self, text: str) -> str:
        """Generate intelligent conclusion from paper content when section not found."""
        text_sample = text[:20000]
        title = self.extract_title(text_sample)
        goal = self.extract_goal(text_sample)
        impact = self.extract_impact(text_sample)
        keywords = self.extract_keywords(text_sample, top_n=5)
        ...
        # Look for explicit findings/results statements
        finding_patterns = [
            r'(?i)(?:findings|results|key findings)[:\s]+([^.!?\n]{20,200})',
            r'(?i)(?:the study (?:found|shows|demonstrates|indicates|reveals))[:\s]+([^.!?\n]{20,200})',
            r'(?i)(?:we (?:found|show|demonstrate|conclude|found that))[:\s]+([^.!?\n]{20,200})',
        ]
        ...
        if goal:
            conclusion_parts.append(f"This research was designed to {goal.lower().rstrip('.')}.")
        
        if findings:
            conclusion_parts.append(f"The study found that {findings[0].lower().rstrip('.')}.")
        
        if impact:
            conclusion_parts.append(f"These findings demonstrate that {impact.lower().rstrip('.')}.")
        
        # Add future work hints
        if any(word in text_lower for word in ['future work', 'future research', 'future direction', 'further work', 'further research']):
            conclusion_parts.append("Future work should explore these findings in greater depth and scale.")
        else:
            if keywords:
                kw_str = ', '.join(keywords[:3])
                conclusion_parts.append(f"The implications extend to research in {kw_str}, offering new perspectives for advancement.")
        
        conclusion = ' '.join(conclusion_parts) if conclusion_parts else ""
        return conclusion if len(conclusion) > 50 else "This research contributes important insights to its field and opens new directions for future investigation."
    ...
    def extract_links(self, text: str) -> List[str]:
        """Extract all links including DOIs, arXiv, URLs, and GitHub repos"""
        cleaned_urls = []
        seen = set()
        
        # Extract URLs (https:// and http://)
        url_pattern = r'https?://[^\s<>"\'\)\]]+(?:\))?'
        ...
        # Extract DOI numbers from plain text format (10.xxxx/...)
        doi_pattern = r'(?:doi\.org|DOI)[:\s]*(10\.\S+)'
        for doi_match in re.finditer(doi_pattern, text, re.IGNORECASE):
            doi = doi_match.group(1).rstrip('.,;:)')
            full_doi = f"DOI: {doi}"
            if full_doi not in seen:
                cleaned_urls.append(full_doi)
                seen.add(full_doi)
        
        # Extract arXiv IDs from plain text
        arxiv_pattern = r'(?:arxiv|arXiv)[:\s]*([\d.]+(?:v\d+)?)'
        for arxiv_match in re.finditer(arxiv_pattern, text):
            arxiv_id = arxiv_match.group(1)
            full_arxiv = f"arXiv: {arxiv_id}"
            if full_arxiv not in seen:
                cleaned_urls.append(full_arxiv)
                seen.add(full_arxiv)
        
        return cleaned_urls[:50]

    def extract_references(self, text: str) -> List[str]:
        """Extract complete references from Bibliography/References section"""
        references = []
        ref_section = ""
        ...
        ref_patterns = [
            ...
            r'(?i)(?:^|\n)\s*(?:references|bibliography)\s*\n+([\s\S]{300,30000}?)(?=\n\s*(?:\[|A|B|C|D|E|References|REFERENCES)|\Z)',
        ]
        ...
        for line in raw_lines:
            ...
            # Check if this line starts a new reference (e.g., [1], 1., (1), [Author], etc.)
            new_ref_indicators = [
                re.match(r'^(\[?\d+[.\)\]]|\(\d+\))', line),  # [1], 1., (1)
                re.match(r'^[A-Z][a-z]+,?\s+[A-Z]', line),   # Last, First or Author Name format
                line.startswith('• ') or line.startswith('- '),  # Bullet points
            ]
            
            if any(new_ref_indicators):
                if current_ref and len(current_ref.split()) >= 3:
                    references.append(current_ref[:300])
                # Remove reference number/marker
                current_ref = re.sub(r'^(\[?\d+[.\)\]]|\(\d+\))\s*', '', line).strip()
                current_ref = re.sub(r'^[•\-\s]+', '', current_ref).strip()
            else:
                # Continuation of current reference
                if current_ref and line:
                    current_ref += " " + line
                elif not current_ref and line:
                    current_ref = line
        ...
        for r in references:
            r = r.strip()
            # Must be meaningful: 25+ chars and 3+ words, likely contains author/journal/year patterns
            word_count = len(r.split())
            if len(r) > 20 and word_count >= 3:
                # Skip if it's just a header or placeholder
                if not re.match(r'^\d+\s*$|^Page|^Chapter|^\s*$', r):
                    cleaned_refs.append(r)
        
        return cleaned_refs[:50]
    
    def extract_page_info(self, text: str) -> dict:
        """Extract page numbers and page ranges from text"""
        pages = {'first_page': None, 'last_page': None, 'total_pages': None, 'page_count': 0}
        
        # Look for "pp. X-Y" or "pages X-Y" patterns
        page_range_patterns = [
            r'(?:pp|pages?|p\.)\s*[:\s]*(\d+)\s*[-–—]\s*(\d+)',
            r'(?:from|pp|pages?)\s*(\d+)\s+to\s+(\d+)',
        ]
        ...
        for pattern in page_range_patterns:
            ...
            if matches:
                ...
                pages['first_page'] = int(first)
                pages['last_page'] = int(last)
                pages['page_count'] = max(0, int(last) - int(first) + 1)
                break
        
        # Look for "total pages" or "# pages"
        total_patterns = [
            r'(\d+)\s+(?:pages?|pp)',
            r'total\s+pages?[:\s]*(\d+)',
        ]
        ...
        for pattern in total_patterns:
            ...
            if matches:
                for match in matches:
                    page_num = int(match) if isinstance(match, str) else int(match[0]) if isinstance(match, tuple) else 0
                    if 5 < page_num < 500:
                        pages['total_pages'] = page_num
                        break
    ...
    def generate_summary(self, text: str, max_length: int = 900, min_length: int = 200) -> str:
        """Generate intelligent summary combining title, abstract, goal, and findings"""
        self._load_models()
        ...
        existing_summary = self.extract_native_summary(text)
        if existing_summary and len(existing_summary) > min_length:
            return existing_summary[:max_length]
        
        # Generate intelligent summary from key sections
        title = self.extract_title(text)
        goal = self.extract_goal(text)
        impact = self.extract_impact(text)
        
        # Build summary from extracted components
        summary_parts = []
        
        if title and title != "Untitled":
            summary_parts.append(f"This research paper, '{title}', ")
        else:
            summary_parts.append("This research paper ")
        
        if goal:
            summary_parts.append(f"aims to {goal.lower().rstrip('.')}. ")
        
        # Add impact/findings
        if impact:
            summary_parts.append(f"The study demonstrates that {impact.lower().rstrip('.')}. ")
        
        # If we have a good summary so far, use it
    ...
    def _extractive_summary(self, text: str, max_length: int = 900) -> str:
        if len(text) > 15000:
            text = text[:15000]
    ...
    def _basic_summary(self, text: str, max_length: int = 150) -> str:
        ...
        top_sentences = [s[1] for s in scored[:6]]
        top_sentences.sort(key=lambda x: sentences.index(x))
        return ' '.join(top_sentences[:5])[:max_length]
    ...
    def full_analysis(self, text: str) -> Dict:
        ...
        return {
            ...
            'summary': self.generate_summary(text[:15000]),
            ...
            'page_info': self.extract_page_info(text),
