    ...
    function resetButton() {
      if (submitBtn) {
        ...
        if (btn) {
          btn.disabled = false;
          btn.innerHTML =
            '<i class="fas fa-wand-magic-sparkles me-2"></i>Analyze Paper';
        }
      }
    }
...
function displayResultsWithTabs(analysis) {
  ...
  const linksHtml = links.slice(0, 15).length
    ? links
        ...
        .map(
          (link) =>
            `<li><a href="${escapeAttr(link)}" target="_blank" rel="noopener noreferrer">${escapeHtml(link)}</a></li>`,
        )
        .join("")
...
// Separate function for escaping attribute values (URLs, etc.)
